...
B = EMOJI_CHAR  # Сокращение для кнопок

def msg(text, *emoji_keys):
    """
    Собирает текст с премиум-эмодзи.
    {key} в тексте заменяется на премиум-эмодзи.
    emoji_keys — порядок ключей, соответствующих {} в тексте.
    """
    result = ""
    entities = []
    emoji_idx = 0
    ...
    while i < len(text):
        if text[i] == '{' and '}' in text[i:]:
            ...
            if key in EID and emoji_idx < len(emoji_keys):
                actual_key = emoji_keys[emoji_idx]
                emoji_char = EMOJI_CHAR.get(actual_key, '')
                entities.append(MessageEntity(
                    type="custom_emoji",
                    offset=len(result),
                    length=len(emoji_char),
                    custom_emoji_id=str(EID[actual_key])  # ← СТРОКА!
                ))
                result += emoji_char
                emoji_idx += 1
                i = end + 1
...
async def send_msg(to, text_template, *emoji_keys, reply_markup=None):
    ...
    if isinstance(to, Message):
        await to.answer(**kwargs)
    elif isinstance(to, CallbackQuery):
        try:
            await to.message.edit_text(**kwargs)
        except Exception:
            await to.message.answer(**kwargs)
    else:
        chat_id = to if isinstance(to, int) else to
        await bot.send_message(chat_id, **kwargs)

async def send_html(to, text, reply_markup=None):
    """Отправляет HTML-сообщение без премиум-эмодзи."""
    if isinstance(to, Message):
        await to.answer(text, reply_markup=reply_markup)
    elif isinstance(to, CallbackQuery):
        try:
            await to.message.edit_text(text, reply_markup=reply_markup)
        except Exception:
            await to.message.answer(text, reply_markup=reply_markup)
    else:
        await bot.send_message(to, text, reply_markup=reply_markup)
...
async def cb_enter_promo(callback: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(OrderStates.entering_promo); await callback.answer()
...
async def cb_referral(callback: CallbackQuery):
    ...
    u = get_user(callback.from_user.id)
    await send_msg(callback,
        f"{{ref}} <b>РЕФЕРАЛЬНАЯ СИСТЕМА</b>\n\nПриглашай друзей и получай <b>{REF_BONUS_GMP} GMP</b> за каждого!\n\n"
        f"🔗 Твоя ссылка:\n<code>{link}</code>\n\n"
...
async def cb_support(callback: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(SupportStates.waiting_message); await callback.answer()
...
async def receive_support_message(message: Message, state: FSMContext):
    ...
    await send_msg(message, "{chat} <b>Сообщение #{t} отправлено!</b>\n{loading} Ожидайте ответа в течение 24 часов.".replace("{t}", str(tid)),
        "chat", "loading", reply_markup=main_menu())
...
async def cb_admin_orders(callback: CallbackQuery):
    ...
    for o in orders:
        ...
        await send_html(callback.message.chat.id,
            ...
            f"💎 {o['gmp_amount']} GMP | 💰 {o['price_rub']} ₽ | ⭐ {o['price_stars']} ⭐\n"
...
async def cb_reply_ticket_start(callback: CallbackQuery, state: FSMContext):
    ...
    await send_msg(callback, "{write} Введи ответ на тикет <b>#{t}</b>:".replace("{t}", str(tid)), "write",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
...
async def process_reply(message: Message, state: FSMContext):
    ...
    await send_msg(message, "{approve} Ответ на тикет <b>#{t}</b> отправлен.".replace("{t}", str(tid)), "approve", reply_markup=admin_menu())
    await state.clear()
