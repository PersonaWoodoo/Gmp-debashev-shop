...
# Символы эмодзи (для кнопок и текста)
EMOJI_CHAR = {
...
def build_entities(text, emoji_positions):
    """
    Создаёт список MessageEntity для премиум-эмодзи.
    text: итоговый текст
    emoji_positions: список кортежей (позиция, ключ_эмодзи)
    """
    entities = []
    for pos, key in emoji_positions:
        entities.append(MessageEntity(
            type="custom_emoji",
            offset=pos,
            length=len(EMOJI_CHAR[key]),
            custom_emoji_id=EID[key]
        ))
    return entities

def msg(text, *emoji_keys):
    """
    Собирает текст с премиум-эмодзи.
    Использование: msg("Привет, {sparkle} как дела? {diamond} круто", "sparkle", "diamond")
    В тексте метки: {key} — будут заменены на соответствующие премиум-эмодзи.
    Очерёдность emoji_keys должна совпадать с очерёдностью {} в тексте.
    """
    result = ""
    entities = []
    emoji_idx = 0
    ...
    while i < len(text):
        if text[i] == '{' and '}' in text[i:]:
            ...
            key = text[i+1:end]
            if key in EID and emoji_idx < len(emoji_keys):
                # Используем ключ из аргументов
                actual_key = emoji_keys[emoji_idx] if emoji_idx < len(emoji_keys) else key
                emoji_char = EMOJI_CHAR.get(actual_key, '')
                entities.append(MessageEntity(
                    type="custom_emoji",
                    offset=len(result),
                    length=len(emoji_char),
                    custom_emoji_id=EID[actual_key]
                ))
                result += emoji_char
                emoji_idx += 1
                i = end + 1
...
async def send_msg(to, text_template, *emoji_keys, reply_markup=None):
    """Отправляет сообщение с премиум-эмодзи."""
    text, entities = msg(text_template, *emoji_keys)
    if isinstance(to, Message):
        await to.answer(text, entities=entities if entities else None, reply_markup=reply_markup)
    elif isinstance(to, CallbackQuery):
        await to.message.edit_text(text, entities=entities if entities else None, reply_markup=reply_markup)
    else:
        await bot.send_message(to, text, entities=entities if entities else None, reply_markup=reply_markup)
...
# ========== КЛАВИАТУРЫ (обычные эмодзи в кнопках) ==========
B = EMOJI_CHAR
...
async def cmd_start(message: Message):
    ...
    if is_banned(user.id):
        await send_msg(message, "{ban} <b>Вы заблокированы в этом боте.</b>", "ban")
        return
    ...
    await send_msg(message,
        ...
        "sparkle", "sparkle", "diamond", "rocket", "lightning", "home",
...
async def cb_buy_gmp(callback: CallbackQuery, state: FSMContext):
    ...
    await send_msg(callback,
        "{cart} <b>КАЛЬКУЛЯТОР GMP</b>\n\n"
        f"{{pin}} Курс: 1 GMP = {gmp_r} ₽\n"
        f"{{star}} Звёзды: 1 ⭐ = {star_r} ₽\n\n"
        f"{{numbers}} Мин. заказ: <b>{MIN_GMP_ORDER} GMP</b>\n"
        f"{{star}} Мин. звёздами: <b>{MIN_STAR_ORDER} ⭐</b>\n\n"
        "{write} Введи сумму GMP <b>в чат</b> (числом):",
        "cart", "pin", "star", "numbers", "star", "write",
...
async def cb_profile(callback: CallbackQuery):
    ...
    if not u: await callback.answer("❌ Профиль не найден"); return
    await send_msg(callback,
        ...
        "profile", "calendar", "box", "diamond", "money", "star", "ref", "ticket",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
...
async def cb_enter_promo(callback: CallbackQuery, state: FSMContext):
    await send_msg(callback, "{ticket} Введи промокод в чат:", "ticket", reply_markup=cancel_inline())
...
async def process_promo(message: Message, state: FSMContext):
    ...
    if err:
        await send_msg(message, "{cancel} {err}", "cancel")
    else:
        await send_msg(message,
            "{approve} <b>Промокод активирован!</b>\n{gift} Бонус: <b>+{p}%</b> к следующему заказу!".replace("{p}", str(promo['bonus_percent'])),
            "approve", "gift"
        )
    await state.clear()
...
async def receive_support_message(message: Message, state: FSMContext):
    ...
    await send_msg(message,
        "{chat} <b>Сообщение #{tid} отправлено!</b>\n{loading} Ожидайте ответа в течение 24 часов.".replace("{tid}", str(tid)),
        "chat", "loading",
        reply_markup=main_menu()
    )
    with suppress(Exception):
        await send_msg(ADMIN_ID,
            "{chat} <b>Новый тикет #{tid}</b>\n{profile} От: @{un}\n{write} Текст: {txt}"
            .replace("{tid}", str(tid)).replace("{un}", message.from_user.username or str(message.from_user.id)).replace("{txt}", message.text[:300]),
            "chat", "profile", "write",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
...
async def cb_bonuses(callback: CallbackQuery):
    await send_msg(callback,
        ...
        "{money} <b>Размер бонуса:</b> {BONUS_PERCENT}% от GMP\n\n"
        "{approve} Бонус начисляется автоматически при проверке заказа!\n\n{lightning} Акция действует постоянно"
        .replace("{BONUS_PERCENT}", str(BONUS_PERCENT)),
        "gift", "sparkle", "money", "approve", "lightning",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text=f"{B['back']} Назад", callback_data="back_menu")]
        ])
    )
...
async def process_gmp_input(message: Message, state: FSMContext):
    ...
    try:
        ...
        if gmp < MIN_GMP_ORDER:
            await send_msg(message, "{cancel} Минимальный заказ: <b>{m} GMP</b>".replace("{m}", str(MIN_GMP_ORDER)), "cancel"); return
        ...
        await send_msg(message,
            ...
            "diamond", "money", "star", reply_markup=payment_menu()
        )
        await state.set_state(OrderStates.choosing_payment)
    except ValueError: await send_msg(message, "{cancel} Введи число!", "cancel")

@dp.callback_query(F.data == "calc_rub", OrderStates.entering_gmp)
async def cb_calc_rub(callback: CallbackQuery, state: FSMContext):
    await send_msg(callback, "{money} Введи сумму в рублях:", "money", reply_markup=cancel_inline())
...
async def process_rub_input(message: Message, state: FSMContext):
    ...
    try:
        ...
        if gmp < MIN_GMP_ORDER:
            await send_msg(message, "{cancel} Слишком мало. Минимум: <b>{m} GMP</b>".replace("{m}", str(MIN_GMP_ORDER)), "cancel"); return
        await state.update_data(gmp_amount=gmp, price_rub=rub, price_stars=stars)
        await send_msg(message,
...
async def cb_calc_stars(callback: CallbackQuery, state: FSMContext):
    await send_msg(callback, "{star} Введи количество звёзд:", "star", reply_markup=cancel_inline())
...
async def process_stars_input(message: Message, state: FSMContext):
    ...
    try:
        ...
        if stars < MIN_STAR_ORDER:
            await send_msg(message, "{cancel} Минимум: <b>{m} ⭐</b>".replace("{m}", str(MIN_STAR_ORDER)), "cancel"); return
        ...
        await send_msg(message,
            ...
            "diamond", "star", "money", reply_markup=payment_menu()
        )
        await state.set_state(OrderStates.choosing_payment)
    except ValueError: await send_msg(message, "{cancel} Введи целое число!", "cancel")
...
async def receive_screenshot(message: Message, state: FSMContext):
    ...
    if not oid: await send_msg(message, "{cancel} Ошибка заказа. Начни заново /start", "cancel"); await state.clear(); return
    conn = get_db(); c = conn.cursor()
    ...
    await send_msg(message,
        ...
        "gallery", "loading", reply_markup=main_menu()
    )
    with suppress(Exception):
        await send_msg(ADMIN_ID,
        ...
        await bot.send_photo(ADMIN_ID, message.photo[-1].file_id, caption=f"🖼 Скриншот заказа #{oid}")
    await state.clear()
...
async def wait_screenshot_text(message: Message, state: FSMContext):
    ...
    await state.clear()
    await send_msg(message, "{cancel} Заказ отменён.", "cancel", reply_markup=main_menu())
...
async def cb_cancel_order(callback: CallbackQuery, state: FSMContext):
    ...
    if 'order_id' in data: update_order(data['order_id'], 'cancelled', 'Отменён пользователем')
    await state.clear()
    await send_msg(callback, "{cancel} <b>Заказ отменён.</b>", "cancel", reply_markup=main_menu())
    await callback.answer()

# ========== АДМИН-ПАНЕЛЬ ==========
@dp.message(Command("admin"))
async def cmd_admin(message: Message):
    if message.from_user.id != ADMIN_ID: await send_msg(message, "{ban} Нет доступа.", "ban"); return
    await send_msg(message, "{admin} <b>АДМИН-ПАНЕЛЬ</b>\n{write} Выберите действие:", "admin", "write", reply_markup=admin_menu())
...
async def cb_admin_stats(callback: CallbackQuery):
    ...
    await send_msg(callback,
        ...
        "stats", "profile", "ban", "loading", "approve", "cancel", "diamond", "gift", "money", "star", "box", "chat",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
...
async def cb_admin_orders(callback: CallbackQuery):
    ...
    if not orders:
        await send_msg(callback, "{box} Нет ожидающих заказов.", "box",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text=f"{B['back']} Назад", callback_data="admin_back")]
            ])
        )
        await callback.answer(); return
    await send_msg(callback, "{loading} Загружаю заказы...", "loading")
    for o in orders:
        o = dict(o)
        await send_msg(callback.message.chat.id,
...
async def cb_admin_tickets(callback: CallbackQuery):
    ...
    if not tickets:
        await send_msg(callback, "{chat} Нет открытых тикетов.", "chat",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text=f"{B['back']} Назад", callback_data="admin_back")]
            ])
        ); await callback.answer(); return
    await send_msg(callback, "{loading} Загружаю тикеты...", "loading")
    for t in tickets:
        await send_msg(callback.message.chat.id,
...
async def cb_reply_ticket_start(callback: CallbackQuery, state: FSMContext):
    ...
    await send_msg(callback, "{write} Введи ответ на тикет <b>#{tid}</b>:".replace("{tid}", str(tid)), "write",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
...
async def process_reply(message: Message, state: FSMContext):
    ...
    await send_msg(message, "{approve} Ответ на тикет <b>#{tid}</b> отправлен.".replace("{tid}", str(tid)), "approve", reply_markup=admin_menu())
    await state.clear()
...
async def set_rate_gmp(message: Message, state: FSMContext):
    ...
    try:
        ...
        await send_msg(message, "{approve} 1 GMP = {r} ₽\n{write} Теперь введи курс звёзд (1 ⭐ = X ₽):".replace("{r}", str(r)), "approve", "write")
...
async def set_rate_star(message: Message, state: FSMContext):
    ...
    try:
        ...
        await send_msg(message, "{approve} 1 ⭐ = {r} ₽\n{lightning} Курсы обновлены!".replace("{r}", str(r)), "approve", "lightning", reply_markup=admin_menu())
        await state.clear()
    except ValueError: await send_msg(message, "{cancel} Введи число!", "cancel")
...
async def set_promo_code(message: Message, state: FSMContext):
    ...
    await send_msg(message, "{write} Введи процент бонуса (число):", "write")
...
async def set_promo_percent(message: Message, state: FSMContext):
    ...
    try:
        ...
        await send_msg(message, "{write} Введи макс. число использований:", "write")
...
async def set_promo_uses(message: Message, state: FSMContext):
    ...
    try:
        ...
        await send_msg(message,
            ...
            "approve", "ticket", "gift", "numbers", "calendar", reply_markup=admin_menu()
        ); await state.clear()
    except ValueError: await send_msg(message, "{cancel} Введи целое число!", "cancel")
...
async def cb_admin_broadcast(callback: CallbackQuery, state: FSMContext):
    if callback.from_user.id != ADMIN_ID: return
    await send_msg(callback, "{bell} <b>РАССЫЛКА</b>\n\n{write} Введи текст для рассылки:", "bell", "write",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text=f"{B['back']} Назад", callback_data="admin_back")]
        ])
...
async def do_broadcast(message: Message, state: FSMContext):
    ...
    await send_msg(message, "{loading} Начинаю рассылку на {n} пользователей...".replace("{n}", str(len(users))), "loading")
    sent = 0
    for u in users:
        with suppress(Exception):
    ...
    await send_msg(message,
        "{approve} <b>Рассылка завершена!</b>\nОтправлено: <b>{s}</b> / {n}".replace("{s}", str(sent)).replace("{n}", str(len(users))),
        "approve", reply_markup=admin_menu()
    ); await state.clear()
...
async def do_ban(message: Message, state: FSMContext):
    ...
    try:
        ...
        if not u: await send_msg(message, "{cancel} Пользователь не найден.", "cancel"); return
        if u['banned']:
            unban_user_db(uid)
            await send_msg(message, "{approve} Пользователь <b>{uid}</b> разбанен.".replace("{uid}", str(uid)), "approve", reply_markup=admin_menu())
        else:
            ban_user_db(uid)
            await send_msg(message, "{ban} Пользователь <b>{uid}</b> забанен.".replace("{uid}", str(uid)), "ban", reply_markup=admin_menu())
            try: await bot.send_message(uid, f"🚫 <b>Вы заблокированы в боте.</b>")
            except: pass
        await state.clear()
    except ValueError: await send_msg(message, "{cancel} Введи корректный ID!", "cancel")

@dp.callback_query(F.data == "admin_search")
async def cb_admin_search(callback: CallbackQuery, state: FSMContext):
    if callback.from_user.id != ADMIN_ID: return
    await send_msg(callback, "🔍 <b>ПОИСК</b>\n\n{write} Введи ID пользователя или номер заказа:", "write",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text=f"{B['back']} Назад", callback_data="admin_back")]
        ])
...
async def do_search(message: Message, state: FSMContext):
    ...
    if orders:
        ...
        if tickets:
            ...
            await send_msg(message, text)
        else: await send_msg(message, "{cancel} Ничего не найдено.", "cancel")
...
async def main():
    ...
    logger.info(f"💎 34 премиум-эмодзи загружено через entities")
