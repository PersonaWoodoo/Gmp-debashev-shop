...
def init_db():
    ...
    cursor.execute('''CREATE TABLE IF NOT EXISTS users (
        ...
        has_bio_bonus INTEGER DEFAULT 0)''')
...
def ban_user_db(user_id, reason="Без причины"):
    conn = get_db(); c = conn.cursor()
    ...
    async def wrapper(event, *args, **kwargs):
        ...
        if is_banned(user_id):
            if isinstance(event, CallbackQuery):
                ...
                await event.answer(f"{E['ban']} <b>Вы заблокированы в этом боте.</b>")
            return
...
async def cb_profile(callback: CallbackQuery):
    ...
    if not u:
        await callback.answer("Профиль не найден. Нажми /start"); return
    text = (
...
async def cb_admin_orders(callback: CallbackQuery):
    ...
    orders = c.fetchall(); conn.close()    if not orders:
        await callback.message.edit_text(
...
async def set_rate_gmp(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(
            f"{E['approve']} 1 GMP = {r} ₽\n{E['write']} Теперь введи курс звёзд (1 ⭐ = X ₽):"
        )
...
async def do_ban(message: Message, state: FSMContext):
    ...
    try:
        ...
        if u['banned']:
            unban_user_db(uid)
            await message.answer(f"{E['approve']} Пользователь <b>{uid}</b> разбанен.", reply_markup=admin_menu())
        else:
            ban_user_db(uid)
            await message.answer(
...
async def unknown_message(message: Message, state: FSMContext):
    ...
    if current_state:
        return  # Не мешаем активным состояниям
    await message.answer(
        f"{E['home']} Используйте кнопки меню для навигации.\n"
        f"Или нажмите /start для перезапуска.",
        reply_markup=main_menu()
    )
