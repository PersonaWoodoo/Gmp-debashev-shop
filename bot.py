...
def check_promo(code, user_id):
    ...
    promo = c.fetchone()
    if not promo:
        conn.close(); return None, "Промокод не найден"
    promo = dict(promo)
    if promo['used_count'] >= promo['max_uses']:
        conn.close(); return None, "Промокод исчерпан"
    ...
    if c.fetchone():
        conn.close(); return None, "Вы уже использовали этот промокод"
...
async def cb_profile(callback: CallbackQuery):
    ...
    text = (
        ...
        f"ID: <code>{u['user_id']}</code>\nИмя: #{u['first_name'] or u['username']}\n"
...
async def cb_enter_promo(callback: CallbackQuery, state: FSMContext):
    await callback.message.edit_text(f"{E['ticket']} Введи промокод в чат:", reply_markup=cancel_inline())
...
async def process_promo(message: Message, state: FSMContext):
    ...
    if err: await message.answer(f"{E['cancel']} {err}")
    else: await message.answer(f"{E['approve']} <b>Промокод активирован!</b>\n{E['gift']} Бонус: <b>+{promo['bonus_percent']}%</b> к следующему заказу!")
    await state.clear()
...
async def cb_support(callback: CallbackQuery, state: FSMContext):
    if is_banned(callback.from_user.id): await callback.answer("🚫 Вы заблокированы.", show_alert=True); return
    await callback.message.edit_text(
        f"{E['chat']} <b>ПОДДЕРЖКА</b>\n\n{E['write']} Опишите ваш вопрос, и мы ответим в течение <b>24 часов</b>.\nПросто напишите сообщение в чат:",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=f"{E['cancel']} Отмена", callback_data="back_menu")]])
    )
...
async def receive_support_message(message: Message, state: FSMContext):
    ...
    await message.answer(f"{E['chat']} <b>Сообщение #{tid} отправлено!</b>\n{E['loading']} Ожидайте ответа в течение 24 часов.", reply_markup=main_menu())
    with suppress(Exception):
        await bot.send_message(ADMIN_ID, f"{E['chat']} <b>Новый тикет #{tid}</b>\n{E['profile']} От: @{message.from_user.username or message.from_user.id}\n{E['write']} Текст: {message.text[:300]}",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=f"{E['write']} Ответить", callback_data=f"reply_ticket_{tid}")]]))
    await state.clear()
...
async def cb_bonuses(callback: CallbackQuery):
    text = (
        ...
        f"2. В разделе «О себе» (Bio) напиши эту фразу:\n<code>{E['diamond']} {BOT_USERNAME} — Лучший выбор для депа! {E['diamond']}</code>\n"
        f"3. Сделай заказ через бота\n4. Мы автоматически проверим твой профиль и начислим бонус!\n\n"
        f"{E['money']} <b>Размер бонуса:</b> {BONUS_PERCENT}% от GMP\n\n{E['approve']} Бонус начисляется автоматически при проверке заказа!\n\n{E['lightning']} Акция действует постоянно"
    )
    await callback.message.edit_text(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=f"{E['back']} Назад", callback_data="back_menu")]]))
    await callback.answer()

@dp.callback_query(F.data == "lottery")
async def cb_lottery(callback: CallbackQuery):
    await callback.message.edit_text(
        f"{E['jackpot']} <b>ЛОТЕРЕЯ</b>\n\n{E['loading']} Раздел в разработке!\nСкоро здесь можно будет выиграть крутые призы.\n\n{E['megaphone']} Следите за обновлениями в канале!",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=f"{E['back']} Назад", callback_data="back_menu")]])
    )
    await callback.answer()
...
async def cb_back_menu(callback: CallbackQuery, state: FSMContext):
    await state.clear()
    await callback.message.edit_text(f"{E['home']} <b>Главное меню</b>\nВыбери раздел:", reply_markup=main_menu())
    await callback.answer()
...
async def process_gmp_input(message: Message, state: FSMContext):
    ...
    try:
        ...
        if gmp < MIN_GMP_ORDER: await message.answer(f"{E['cancel']} Минимальный заказ: <b>{MIN_GMP_ORDER} GMP</b>"); return
        ...
        await message.answer(f"{E['diamond']} <b>РАСЧЁТ:</b>\n\n💎 GMP: <b>{gmp}</b>\n{E['money']} Рубли: <b>{rub} ₽</b>\n{E['star']} Звёзды: <b>{stars} ⭐</b>\n\nВыбери способ оплаты:", reply_markup=payment_menu())
        await state.set_state(OrderStates.choosing_payment)
    except ValueError: await message.answer(f"{E['cancel']} Введи число!")

@dp.callback_query(F.data == "calc_rub", OrderStates.entering_gmp)
async def cb_calc_rub(callback: CallbackQuery, state: FSMContext):
    await callback.message.edit_text(f"{E['money']} Введи сумму в рублях:", reply_markup=cancel_inline())
...
async def process_rub_input(message: Message, state: FSMContext):
    ...
    try:
        ...
        if gmp < MIN_GMP_ORDER: await message.answer(f"{E['cancel']} Слишком мало. Минимум: <b>{MIN_GMP_ORDER} GMP</b>"); return
        await state.update_data(gmp_amount=gmp, price_rub=rub, price_stars=stars)
        await message.answer(f"{E['diamond']} <b>РАСЧЁТ:</b>\n\n{E['money']} {rub} ₽ = 💎 {gmp} GMP = {E['star']} {stars} ⭐\n\nВыбери способ оплаты:", reply_markup=payment_menu())
        await state.set_state(OrderStates.choosing_payment)
    except ValueError: await message.answer(f"{E['cancel']} Введи число!")
...
async def cb_calc_stars(callback: CallbackQuery, state: FSMContext):
    await callback.message.edit_text(f"{E['star']} Введи количество звёзд:", reply_markup=cancel_inline())
...
async def process_stars_input(message: Message, state: FSMContext):
    ...
    try:
        ...
        if stars < MIN_STAR_ORDER: await message.answer(f"{E['cancel']} Минимум: <b>{MIN_STAR_ORDER} ⭐</b>"); return
        star_r = float(get_setting('star_rate')); gmp_r = float(get_setting('gmp_rate'))
        rub = round(stars * star_r, 2); gmp = round(rub / gmp_r, 2)
        await state.update_data(gmp_amount=gmp, price_rub=rub, price_stars=stars)
        await message.answer(f"{E['diamond']} <b>РАСЧЁТ:</b>\n\n{E['star']} {stars} ⭐ = {E['money']} {rub} ₽ = 💎 {gmp} GMP\n\nВыбери способ оплаты:", reply_markup=payment_menu())
        await state.set_state(OrderStates.choosing_payment)
    except ValueError: await message.answer(f"{E['cancel']} Введи целое число!")
...
async def cb_pay_rub(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        f"{E['bank']} <b>ЗАКАЗ #{oid}</b>\n\n{E['money']} Сумма: <b>{data['price_rub']} ₽</b>\n{E['diamond']} Получите: <b>{data['gmp_amount']} GMP</b>\n\n{E['gallery']} После оплаты отправьте <b>скриншот</b> в этот чат.\nАдмин проверит и начислит GMP.",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
...
async def cb_pay_stars(callback: CallbackQuery, state: FSMContext):
    ...
    await state.update_data(order_id=oid)
    await callback.message.edit_text(
        f"{E['star']} <b>ЗАКАЗ #{oid}</b>\n\nОтправьте <b>{stars} ⭐</b> подарком на {SUPPORT_USERNAME}\n{E['diamond']} Получите: <b>{data['gmp_amount']} GMP</b>\n\n{E['gallery']} После отправки пришлите <b>скриншот</b> в этот чат.",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=f"{E['cancel']} Отменить заказ", callback_data="cancel_order")]])
    )
    await state.set_state(OrderStates.waiting_screenshot); await callback.answer()
...
async def receive_screenshot(message: Message, state: FSMContext):
    ...
    if not oid: await message.answer(f"{E['cancel']} Ошибка заказа. Начни заново /start"); await state.clear(); return
    conn = get_db(); c = conn.cursor()
    c.execute('UPDATE orders SET screenshot_id=? WHERE order_id=?', (message.photo[-1].file_id, oid))
    conn.commit(); conn.close()
    await message.answer(f"{E['gallery']} <b>Скриншот получен!</b>\n{E['loading']} Заказ <b>#{oid}</b> ожидает проверки.\nОбычно это занимает 5-10 минут.", reply_markup=main_menu())
    with suppress(Exception):
        await bot.send_message(ADMIN_ID, f"{E['bell']} <b>Новый скрин по заказу #{oid}</b>\n{E['profile']} От: @{message.from_user.username or message.from_user.id}\n{E['money']} Сумма: {data.get('price_rub', '?')} ₽",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=f"{E['approve']} Подтвердить", callback_data=f"approve_{oid}"), InlineKeyboardButton(text=f"{E['cancel']} Отменить", callback_data=f"reject_{oid}")]]))
        await bot.send_photo(ADMIN_ID, message.photo[-1].file_id, caption=f"{E['gallery']} Скриншот заказа #{oid}")
    await state.clear()

@dp.message(OrderStates.waiting_screenshot)
async def wait_screenshot_text(message: Message, state: FSMContext):
    if message.text == f"{E['cancel']} Отменить заказ":
        data = await state.get_data()
        if 'order_id' in data: update_order(data['order_id'], 'cancelled', 'Отменён пользователем')
        await state.clear(); await message.answer(f"{E['cancel']} Заказ отменён.", reply_markup=main_menu())
    else: await message.answer(f"{E['gallery']} Пожалуйста, отправьте <b>скриншот</b> (фото).")
...
async def cb_cancel_order(callback: CallbackQuery, state: FSMContext):
    ...
    if 'order_id' in data: update_order(data['order_id'], 'cancelled', 'Отменён пользователем')
    await state.clear()
    await callback.message.edit_text(f"{E['cancel']} <b>Заказ отменён.</b>", reply_markup=main_menu())
    await callback.answer()

# ========== АДМИН-ПАНЕЛЬ ==========
@dp.message(Command("admin"))
async def cmd_admin(message: Message):
    if message.from_user.id != ADMIN_ID: await message.answer(f"{E['ban']} Нет доступа."); return
    await message.answer(f"{E['admin']} <b>АДМИН-ПАНЕЛЬ</b>\n{E['write']} Выберите действие:", reply_markup=admin_menu())
...
async def cb_admin_stats(callback: CallbackQuery):
    ...
    await callback.message.edit_text(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=f"{E['back']} Назад", callback_data="admin_back")]]))
    await callback.answer()
...
async def cb_admin_orders(callback: CallbackQuery):
    ...
    if not orders:
        await callback.message.edit_text(f"{E['box']} Нет ожидающих заказов.", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=f"{E['back']} Назад", callback_data="admin_back")]]))
        await callback.answer(); return
    await callback.message.edit_text(f"{E['loading']} Загружаю заказы...")
    for o in orders:
        o = dict(o)
        text = f"{E['box']} <b>Заказ #{o['order_id']}</b>\n{E['profile']} @{o['username']} (ID: <code>{o['user_id']}</code>)\n{E['diamond']} {o['gmp_amount']} GMP | {E['money']} {o['price_rub']} ₽ | {E['star']} {o['price_stars']} ⭐\n{E['card']} Метод: {o['payment_method']}\n{E['calendar']} {o['created_at']}"
...
async def cb_approve(callback: CallbackQuery):
    ...
    if o:
        with suppress(Exception): await bot.send_message(o['user_id'], f"{E['approve']} <b>Заказ #{oid} выполнен!</b>\n{E['diamond']} Начислено: <b>{o['gmp_amount']} GMP</b>\nСпасибо за покупку! {E['sparkle']}")
...
async def cb_reject(callback: CallbackQuery):
    ...
    if o:
        with suppress(Exception): await bot.send_message(o['user_id'], f"{E['cancel']} <b>Заказ #{oid} отклонён.</b>\n{E['chat']} Обратитесь в поддержку: {SUPPORT_USERNAME}")
...
async def cb_admin_tickets(callback: CallbackQuery):
    ...
    if not tickets:
        await callback.message.edit_text(f"{E['chat']} Нет открытых тикетов.", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=f"{E['back']} Назад", callback_data="admin_back")]]))
        await callback.answer(); return
    await callback.message.edit_text(f"{E['loading']} Загружаю тикеты...")
    for t in tickets:
        text = f"{E['chat']} <b>Тикет #{t['ticket_id']}</b>\n{E['profile']} @{t['username']} (ID: {t['user_id']})\n{E['write']} {t['message'][:300]}\n{E['calendar']} {t['created_at']}"
        await callback.message.answer(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=f"{E['write']} Ответить", callback_data=f"reply_ticket_{t['ticket_id']}")]]))
...
async def cb_reply_ticket_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(f"{E['write']} Введи ответ на тикет <b>#{tid}</b>:", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=f"{E['cancel']} Отмена", callback_data="admin_back")]]))
...
async def process_reply(message: Message, state: FSMContext):
    ...
    data = await state.get_data(); tid = data['reply_tid']
    reply_ticket(tid, message.text)
    conn = get_db(); c = conn.cursor()
    c.execute('SELECT user_id FROM support_tickets WHERE ticket_id=?', (tid,)); t = c.fetchone(); conn.close()
    if t:
        with suppress(Exception): await bot.send_message(t['user_id'], f"{E['chat']} <b>Ответ поддержки (тикет #{tid}):</b>\n\n{message.text}\n\n{E['write']} Если остались вопросы — создайте новый тикет.")
    await message.answer(f"{E['approve']} Ответ на тикет <b>#{tid}</b> отправлен.", reply_markup=admin_menu()); await state.clear()
...
async def cb_admin_rate(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(f"{E['money']} <b>Текущие курсы:</b>\n{E['diamond']} 1 GMP = <b>{gmp_r} ₽</b>\n{E['star']} 1 ⭐ = <b>{star_r} ₽</b>\n\n{E['write']} Введи новый курс GMP (в рублях):", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=f"{E['back']} Назад", callback_data="admin_back")]]))
...
async def set_rate_gmp(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(f"{E['approve']} 1 GMP = {r} ₽\n{E['write']} Теперь введи курс звёзд (1 ⭐ = X ₽):"); await state.set_state(AdminStates.waiting_rate_star)
    except ValueError: await message.answer(f"{E['cancel']} Введи число!")
...
async def set_rate_star(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(f"{E['approve']} 1 ⭐ = {r} ₽\n{E['lightning']} Курсы обновлены!", reply_markup=admin_menu()); await state.clear()
    except ValueError: await message.answer(f"{E['cancel']} Введи число!")
...
async def cb_admin_req(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(f"{E['card']} <b>Текущие реквизиты:</b>\n{r[:500]}\n\n{E['write']} Введи новые реквизиты:", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=f"{E['back']} Назад", callback_data="admin_back")]]))
...
async def set_req(message: Message, state: FSMContext):
    ...
    await message.answer(f"{E['approve']} Реквизиты обновлены!", reply_markup=admin_menu()); await state.clear()

@dp.callback_query(F.data == "admin_promo")
async def cb_admin_promo(callback: CallbackQuery, state: FSMContext):
    if callback.from_user.id != ADMIN_ID: return
    await callback.message.edit_text(f"{E['ticket']} <b>СОЗДАНИЕ ПРОМОКОДА</b>\n\n{E['write']} Введи код (латиница/цифры):", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=f"{E['back']} Назад", callback_data="admin_back")]]))
...
async def set_promo_code(message: Message, state: FSMContext):
    ...
    await message.answer(f"{E['write']} Введи процент бонуса (число, например 10):"); await state.set_state(AdminStates.waiting_promo_percent)
...
async def set_promo_percent(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(f"{E['write']} Введи максимальное число использований:"); await state.set_state(AdminStates.waiting_promo_uses)
    except ValueError: await message.answer(f"{E['cancel']} Введи целое число!")
...
async def set_promo_uses(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(f"{E['approve']} <b>Промокод создан!</b>\n{E['ticket']} Код: <b>{data['promo_code']}</b>\n{E['gift']} Бонус: <b>{data['promo_percent']}%</b>\n{E['numbers']} Использований: <b>{uses}</b>\n{E['calendar']} Срок: 72 часа", reply_markup=admin_menu()); await state.clear()
    except ValueError: await message.answer(f"{E['cancel']} Введи целое число!")

@dp.callback_query(F.data == "admin_broadcast")
async def cb_admin_broadcast(callback: CallbackQuery, state: FSMContext):
    if callback.from_user.id != ADMIN_ID: return
    await callback.message.edit_text(f"{E['bell']} <b>РАССЫЛКА</b>\n\n{E['write']} Введи текст для рассылки:", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=f"{E['back']} Назад", callback_data="admin_back")]]))
    await state.set_state(AdminStates.waiting_broadcast); await callback.answer()

@dp.message(AdminStates.waiting_broadcast)
async def do_broadcast(message: Message, state: FSMContext):
    if message.from_user.id != ADMIN_ID: return
    conn = get_db(); c = conn.cursor(); c.execute('SELECT user_id FROM users WHERE banned=0'); users = c.fetchall(); conn.close()
    await message.answer(f"{E['loading']} Начинаю рассылку на {len(users)} пользователей...")
    sent = 0
    for u in users:
        with suppress(Exception): await bot.send_message(u['user_id'], f"{E['bell']} <b>Рассылка:</b>\n\n{message.text}"); sent += 1; await asyncio.sleep(0.05)
    await message.answer(f"{E['approve']} <b>Рассылка завершена!</b>\nОтправлено: <b>{sent}</b> / {len(users)}", reply_markup=admin_menu()); await state.clear()

@dp.callback_query(F.data == "admin_db")
async def cb_admin_db(callback: CallbackQuery):
    if callback.from_user.id != ADMIN_ID: return
    try: await callback.message.answer_document(FSInputFile(DB_NAME), caption=f"{E['download']} База данных бота\n{E['calendar']} {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    except: await callback.message.answer(f"{E['cancel']} Файл базы данных не найден.")
    await callback.answer()

@dp.callback_query(F.data == "admin_ban")
async def cb_admin_ban(callback: CallbackQuery, state: FSMContext):
    if callback.from_user.id != ADMIN_ID: return
    await callback.message.edit_text(f"{E['ban']} <b>БАН ПОЛЬЗОВАТЕЛЯ</b>\n\n{E['write']} Введи ID пользователя:", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=f"{E['back']} Назад", callback_data="admin_back")]]))
...
async def do_ban(message: Message, state: FSMContext):
    ...
    try:
        ...
        if not u: await message.answer(f"{E['cancel']} Пользователь не найден."); return
        if u['banned']: unban_user_db(uid); await message.answer(f"{E['approve']} Пользователь <b>{uid}</b> разбанен.", reply_markup=admin_menu())
        else: ban_user_db(uid); await message.answer(f"{E['ban']} Пользователь <b>{uid}</b> забанен.", reply_markup=admin_menu()); with suppress(Exception): await bot.send_message(uid, f"{E['ban']} <b>Вы заблокированы в боте.</b>")
        await state.clear()
    except ValueError: await message.answer(f"{E['cancel']} Введи корректный ID!")

@dp.callback_query(F.data == "admin_search")
async def cb_admin_search(callback: CallbackQuery, state: FSMContext):
    if callback.from_user.id != ADMIN_ID: return
    await callback.message.edit_text(f"🔍 <b>ПОИСК</b>\n\n{E['write']} Введи ID пользователя или номер заказа:", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=f"{E['back']} Назад", callback_data="admin_back")]]))
...
async def do_search(message: Message, state: FSMContext):
    ...
    if order:
        order = dict(order)
        await message.answer(f"{E['box']} <b>Заказ #{order['order_id']}</b>\n{E['profile']} ID: {order['user_id']}\n@{order['username']}\n{E['diamond']} {order['gmp_amount']} GMP\n{E['money']} {order['price_rub']} ₽ | {E['star']} {order['price_stars']} ⭐\n{E['card']} Метод: {order['payment_method']}\nСтатус: {order['status']}\n{E['calendar']} {order['created_at']}")
    ...
    if orders:
        ...
        for o in orders: o = dict(o); text += f"{E['box']} #{o['order_id']} | {E['diamond']} {o['gmp_amount']} GMP | {o['status']} | {o['created_at']}\n"
        ...
        if tickets:
            ...
            await message.answer(text)
        else: await message.answer(f"{E['cancel']} Ничего не найдено по запросу: {query}")
...
async def cb_admin_back(callback: CallbackQuery, state: FSMContext):
    if callback.from_user.id != ADMIN_ID: return
    await state.clear()
    await callback.message.edit_text(f"{E['admin']} <b>АДМИН-ПАНЕЛЬ</b>\n{E['write']} Выберите действие:", reply_markup=admin_menu())
    await callback.answer()
